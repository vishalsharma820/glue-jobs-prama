...
pipeline {
    ...
    // Dropdown parameter to select one module or "all"
    parameters {
        choice(
            ...
            description: 'Select a module to run plan on (or "all" to run on every module)'
        )
    }
        ...
        stage('Terragrunt Plan via Makefile') {
            steps {
                dir('deploy') {
                    ...
                    echo "Running 'make plan' with MODULE_SELECTION=${MODULE_SELECTION}"
                    make plan MODULE_SELECTION=${MODULE_SELECTION}
                    '''
                }
            }
        }
